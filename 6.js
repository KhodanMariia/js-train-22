...
class AuthProcessor {
  ...
  validate(username, passkey) {
    ...
    if (this.nextProcessor) {
      // Якщо так, передає запит на перевірку аутентифікації наступному обробнику,
      //  this.nextProcessor.validate(username, passkey), та повертаємо результат.
      return this.nextProcessor.validate(username, passkey);
    } else {
      // Якщо наступного обробника немає,
      //  повертає false, сигналізуючи про невдалу аутентифікацію.
      return false;
    }
  }
}
...
class TwoStepProcessor extends AuthProcessor {
  ...
  // Перевіряє ім'я користувача (username), пароль (passkey) і викликаємо метод isValidTwoStepCode().
  validate(username, passkey) {
    // Якщо username дорівнює "john", passkey дорівнює "password" та метод isValidTwoStepCode()
    // повертає true, аутентифікація успішна.
    if (username === "john" && passkey === "password") {
      // Виводить повідомлення про успішну аутентифікацію: Вхід дозволено з двофакторною аутентифікацією,
      console.log("Вхід дозволено з двофакторною аутентифікацією");
      //  і повертає true.
      return true;
    } else {
      // Якщо дані не вірні,
      //  запит на аутентифікацію передається наступному обробнику в ланцюгу, super.validate(username, passkey).
...
class RoleProcessor extends AuthProcessor {
  ...
  validate(username, passkey) {
    ...
    if (username === "guest") {
      // Виводить повідомлення про успішну аутентифікацію Вхід дозволено з роллю гостя,
      console.log("Вхід дозволено з роллю гостя");
      //  і повертає true.
      return true;
    } else {
      // Якщо роль не відповідає,
      // запит на аутентифікацію передається наступному обробнику в ланцюгу.
      return super.validate(username, passkey);
    }
  }
}
